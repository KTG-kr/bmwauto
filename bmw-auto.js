window.runBmwAuto = function () {
  ...
  function getTrim(sec) {
    return [...document.querySelectorAll("div.trim")]
      .find(t => n(t.querySelector("h3") && t.querySelector("h3").innerText) === sec);
  }

  function optionText(a) {
    var alt = n(a.querySelector("img") && a.querySelector("img").getAttribute("alt"));
    var tip = n(a.querySelector(".tooltiptext") && a.querySelector(".tooltiptext").innerText);
    var all = n(a.innerText || a.textContent || "");
    return n(alt + " " + tip + " " + all);
  }
  ...
  function pickOption(sec, wants) {
    ...
    var opts = [...trim.querySelectorAll("a.tooltip")].filter(enabledOption);
